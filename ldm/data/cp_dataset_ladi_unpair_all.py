...
class CPDataset(data.Dataset):
    ...
    def __init__(self, dataroot, image_size=512, mode='train', semantic_nc=13, unpaired=False):
        ...
        folders = []
        c_folders = []
        with open(os.path.join(dataroot, 'unpaired_all.txt'), 'r') as f:
            for line in f.readlines():
                p_name, c_name = line.strip().split()
                folders.append(p_name)
                c_folders.append(c_name)
        self.folders = folders
        self.c_folders = c_folders
    ...
    def __getitem__(self, index):
        ...
        c_names = []
        for c_name in os.listdir(osp.join(self.data_path, 'image-wo-bg', c_folder)):
        ...
        # agnostic image
        inpaint_big = Image.open(osp.join(self.data_path, 'warp_feat_ladi_unpair_all', c_folder + '_' + folder + '.jpg'))
        ...
        # skeletons
        skeleton_cf_big = Image.open(osp.join(self.data_path, 'skeletons', c_folder, c_names[0]))
        skeleton_cf = transforms.Resize(self.crop_size, interpolation=2)(skeleton_cf_big)
        skeleton_cf = self.transform(skeleton_cf)

        skeleton_cb_big = Image.open(osp.join(self.data_path, 'skeletons', c_folder, c_names[2]))
        ...
        # ref_image_f
        c = Image.open(osp.join(self.data_path, 'cloth', c_folder, c_names[0]))
        c = transforms.Resize(self.crop_size, interpolation=2)(c)
        c = self.transform(c)
        controlnet_cond_f = c

        cm = Image.open(osp.join(self.data_path, 'cloth-mask', c_folder, c_names[0]))
        ...
        # ref_image_b
        c = Image.open(osp.join(self.data_path, 'cloth', c_folder, c_names[2]))
        c = transforms.Resize(self.crop_size, interpolation=2)(c)
        c = self.transform(c)
        controlnet_cond_b = c

        cm = Image.open(osp.join(self.data_path, 'cloth-mask', c_folder, c_names[2]))
        cm = transforms.Resize(self.crop_size, interpolation=0)(cm)
        ...
        result = {
            ...
            "file_name": c_folder + '_' + folder + '.jpg'
        }
        return result
