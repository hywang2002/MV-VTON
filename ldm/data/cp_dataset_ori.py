...
class CPDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        im_name = self.im_names[index]  # 10224_00.jpg
        ...
        parse = torch.from_numpy(np.array(im_parse_pil)[None]).long()  # [1,H,W]
        ...
        parse_map = parse_map.scatter_(0, parse, 1.0)  # torch.Size([20, 512, 512])
        ...
        mask = torch.isin(parse_onehot[0], mask_id).numpy()  # 对应位置为 True 表示该位置的元素在 mask_id 中
        ...
        inpaint_mask = 1 - self.toTensor(mask)  # 需要inpainting的区域值为0
        ...
        result = {
            ...
            "ref_imgs": ref_image,  # clip_normalize
            'warp_feat': feat,  # [-1,1]
