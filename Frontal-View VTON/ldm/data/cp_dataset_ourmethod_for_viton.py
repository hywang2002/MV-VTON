# coding=utf-8
# cp_dataset_ourmethod_for_viton.py
import os
...
class CPDataset(data.Dataset):
    ...
    def __init__(self, dataroot, image_size=512, mode='train', semantic_nc=13, unpaired=False):
        ...
        for folder in os.listdir(osp.join(self.data_path, "image")):
            folders.append(folder)
        folders = sorted(folders)  # 00006_00.jpg, 00008_00.jpg ...
    ...
    def __getitem__(self, index):
        im_names = self.folders[index]  # 00006_00.jpg
        # print(im_names)
        # print("===========")
        
        # person image
        im_pil_big = Image.open(osp.join(self.data_path, 'image', im_names))
        im_pil = transforms.Resize(self.crop_size, interpolation=2)(im_pil_big)
        im = self.transform(im_pil)

        # agnostic image
        inpaint_big = Image.open(osp.join(self.data_path, 'agnostic_hy', im_names))
        inpaint = transforms.Resize(self.crop_size, interpolation=2)(inpaint_big)
        inpaint = self.transform(inpaint)

        # inpaint mask
        inpaint_mask_big = Image.open(osp.join(self.data_path, "inpaint_mask_hy", im_names)).convert('L')
        ...
        # ref_image_f
        c = Image.open(osp.join(self.data_path, 'cloth', im_names))
        c = transforms.Resize(self.crop_size, interpolation=2)(c)
        c = self.transform(c)

        cm = Image.open(osp.join(self.data_path, 'cloth-mask', im_names))
...
if __name__ == '__main__':
    dataset = CPDataset('/mnt/pfs-mc0p4k/cvg/team/didonglin/why/datasets/VITON-HD_copy', 512, mode='test', unpaired=False)
